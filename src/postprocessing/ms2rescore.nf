workflow ms2rescore_workflow {
    ...
    spectrum_id_pattern
    searchengine
    // runtime / configuration values passed main.nf
    ms2pip_model_dir
    ms2rescore_model
    ms2rescore_threads
    ms2rescore_mem
    ...
    main:
    ms2pip_model_dir = Channel.fromPath(ms2pip_model_dir, type: 'dir').first()
    check_or_download_model(ms2pip_model_dir, ms2rescore_model)

    ms2rescore_pre_pins = run_chunked_ms2rescore(psm_tsvs_and_mzmls, psm_tsvs, mzmls, spectrum_id_pattern, params.fragment_tol_da, ms2pip_model_dir, ms2rescore_model, ms2rescore_threads, ms2rescore_chunk_size)
    ms2rescore_pins = correct_psm_utils_pins(ms2rescore_pre_pins, searchengine, outdir)
...
process run_chunked_ms2rescore {
    cpus  { ms2rescore_threads }
    memory { ms2rescore_mem }
    ...
    chunked_ms2rescore.py -psms_file ${psm_utils_tsvs} \
        ...
        -model ${ms2rescore_model} -model_dir "${ms2pip_model_dir}" \
        -ms2_tolerance ${fragment_tol_da} \
        -spectrum_id_pattern '${spectrum_id_pattern}' \
        -processes ${ms2rescore_threads} \
